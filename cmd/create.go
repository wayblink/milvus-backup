...
var createBackupCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		if backupUpload {
			start := time.Now().Unix()
			err := backupContext.AutoUploadBackupStore(context, core.BackupDirPath(params.MinioCfg.BackupRootPath, backupName))
			if err != nil {
				fmt.Println("err:", err)
			}
...
func init() {
	...
	createBackupCmd.Flags().BoolVarP(&backupUpload, "backupUpload", "b", false, "upload the s3 files to backup store")
