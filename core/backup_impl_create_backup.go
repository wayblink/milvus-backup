...
func (b *BackupContext) executeCreateBackup(ctx context.Context, request *backuppb.CreateBackupRequest, backupInfo *backuppb.BackupInfo, createCDCTaskPos bool) (*backuppb.BackupInfo, error) {
	...
	b.AutoUploadBackupStore(ctx, BackupDirPath(b.backupRootPath, backupInfo.GetName()))
...
func (b *BackupContext) AutoUploadBackupStore(ctx context.Context, rootPath string) error {
	if !b.params.MinioCfg.BackupCopyEnable {
		return nil
	}
	paths, _, err := b.getStorageClient().ListWithPrefix(ctx, b.backupBucketName, rootPath, true)
	if err != nil {
		log.Warn("Fail to list backup store", zap.Error(err))
		return err
	}
	group, groupCtx := errgroup.WithContext(ctx)
	concurrentNum := int(b.params.MinioCfg.BackupCopyConcurrentNumber)
	if concurrentNum < 1 {
		concurrentNum, _ = strconv.Atoi(paramtable.DefaultBackupCopyConcurrentNumber)
	}
	for i := 0; i < concurrentNum; i++ {
		index := i
		group.Go(func() error {
			for i2, p := range paths {
				select {
				case <-groupCtx.Done():
					return groupCtx.Err()
				default:
					if i2%concurrentNum != index {
						continue
					}
					data, err := b.getStorageClient().Read(groupCtx, b.backupBucketName, p)
					if err != nil {
						log.Warn("Fail to read backup store", zap.Error(err))
						return err
					}
					err = b.getBackupStorageClient().Write(groupCtx, b.backupBucketName, p, data)
					if err != nil {
						log.Warn("Fail to write backup store", zap.Error(err))
						return err
					}
				}
			}
	...
	err = group.Wait()
	if err != nil {
		log.Warn("Fail to upload backup store", zap.Error(err))
		return err
	}
	return nil
}
